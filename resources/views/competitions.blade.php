                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
                        ...
                        @forelse($competitions as $lomba)
                            @php
                                ...
                                $tglBuka = $lomba->tgl_buka_pendaftaran
                                    ? \Carbon\Carbon::parse($lomba->tgl_buka_pendaftaran)->startOfDay()
                                    : $hariIni;
                            ...
                            <div data-tingkat="{{ strtolower($lomba->tingkat_sekolah ?? '') }}"
                                ...
                                <div class="p-6 flex flex-col flex-grow rounded-b-[24px]">
                                    ...
                                    <div class="flex items-start justify-between gap-2 mb-4" x-data="{ showMenu: false }">
                                        <h3 class="text-lg font-black text-slate-900 leading-tight line-clamp-2 h-[3.5rem] flex-grow"
                                    ...
                                    <div class="pt-5 border-t border-slate-100 flex items-center justify-between relative"
                                        ...
                                        <div class="relative">
                                            @if ($belumBuka)
                                                <button type="button"
                                                    class="bg-slate-200 text-slate-500 px-5 py-2 rounded-full text-xs font-bold cursor-not-allowed text-center">
                                                    Belum Dibuka 🔒
                                                </button>
                                            @else
                                                @auth
                                                    @if (auth()->user()->role === 'peserta')
                                                        @if (auth()->user()->isProfileComplete())
                                                            ...
                                                            <div x-show="showDropdown" style="display: none;"
                                                                ...
                                                                @empty
                                                                    <div
                                                                        class="px-4 py-3 text-xs text-slate-500 text-center">
                                                                        Belum ada bidang</div>
                                                                @endforelse
                                                            </div>
                                                        @else
                                                            <button type="button"
                                                                @click="$dispatch('open-profile-alert')"
                                                                class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-full text-sm font-bold flex items-center gap-2 shadow-lg shadow-blue-200 transition-all active:scale-95">
                                                                Daftar
                                                    ...
                                                    @else
                                                        <button type="button"
                                                            ...
                                                            class="bg-slate-400 text-white px-5 py-2 rounded-full text-sm font-bold flex items-center gap-2 shadow-lg cursor-not-allowed">Daftar</button>
                                                    @endif
                                                @else
                                                    <a href="{{ route('login') }}"
                                                        class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-full text-sm font-bold flex items-center gap-2 shadow-lg shadow-blue-200 transition-all active:scale-95">Daftar</a>
                                                @endauth
                                            @endif
                                        </div>
                                    </div>
                ...
                <form action="{{ route('registrations.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="p-6 overflow-y-auto custom-scrollbar flex-grow">
                        <div x-show="price !== 'Gratis'" style="display: none;">
                            <div class="bg-blue-50/40 border border-blue-100 rounded-xl p-5 mb-2">
                                ...
                                <ul class="space-y-2.5">
                                    <li class="flex items-start gap-2.5 text-sm text-slate-600 font-medium"><svg
                                            class="w-5 h-5 text-green-500 flex-shrink-0" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <li class="flex items-start gap-2.5 text-sm text-slate-600 font-medium"><svg
                                            ...
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                                d="M5 13l4 4L19 7"></path>
                                        </svg>Proses registrasi lebih cepat & prioritas</li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div x-show="price === 'Gratis'" style="display: none;" class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div><label class="block text-xs font-bold text-slate-700 mb-1.5">1. Bukti Follow IG
                                        <span class="text-red-500">*</span></label><input type="file"
                                        name="bukti_follow" :required="price === 'Gratis'"
                                ...
                                <div><label class="block text-xs font-bold text-slate-700 mb-1.5">2. Bukti Share Poster
                                        <span class="text-red-500">*</span></label><input type="file"
                                        name="bukti_share" :required="price === 'Gratis'"
                                        accept=".jpg,.jpeg,.png,.pdf"
                                        class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs file:mr-3 file:py-1.5 file:px-3 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                                </div>
                                <div class="md:col-span-2"><label
                                        class="block text-xs font-bold text-slate-700 mb-1.5">3. Bukti Tag/Komen di
                                        Postingan <span class="text-red-500">*</span></label><input type="file"
                                        name="bukti_komentar" :required="price === 'Gratis'"
                                        accept=".jpg,.jpeg,.png,.pdf"
                                        class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs file:mr-3 file:py-1.5 file:px-3 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                                </div>
                            </div>
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-2"><span
                                class="text-sm font-bold text-slate-500">Total:</span><span
                                class="text-2xl font-black"
                                :class="price === 'Gratis' ? 'text-green-600' : 'text-slate-900'"
                                x-text="price"></span></div>
            ...
            <div x-show="isProfileAlertOpen"
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center pt-6 border-t border-slate-100">
                    <button type="button" @click="isProfileAlertOpen = false"
                        class="px-8 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-full transition-colors text-sm active:scale-95">Nanti
                        Saja</button>
