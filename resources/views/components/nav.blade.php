...
<nav class="fixed top-0 w-full z-50 bg-white/30 backdrop-blur-md border-b border-white/40 shadow-sm transition-all duration-300">
    <div class="w-full px-4 md:px-8 lg:px-16 py-4 md:py-5 flex items-center justify-between">
        ...
        <div class="flex items-center gap-6 xl:gap-12">
            ...
            @if (!auth()->check() || auth()->user()->role !== 'admin')
                <div class="hidden lg:flex items-center bg-[#F4F7FE] rounded-full px-5 py-3 w-[270px] border border-transparent focus-within:border-blue-200 focus-within:bg-white transition-all">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                    <input type="search" placeholder="Search competitions ..." class="search-input bg-transparent border-none focus:outline-none focus:ring-0 text-sm w-full ml-3 text-slate-700 placeholder-slate-400">
                </div>

                <div class="hidden lg:flex items-center space-x-2 absolute left-1/2 transform -translate-x-1/2">
                    <a href="{{ $dashboardUrl }}" class="{{ $isDashboardActive ? 'bg-blue-600 text-white px-6 shadow-lg shadow-blue-500/20' : 'text-slate-600 hover:text-blue-600 hover:bg-blue-50 px-5' }} py-3 rounded-full font-bold text-base transition-all duration-300">
                        Dashboard
                    </a>
                    <a href="{{ $competitionsUrl }}" class="{{ $isCompetitionsActive ? 'bg-blue-600 text-white px-6 shadow-lg shadow-blue-500/20' : 'text-slate-600 hover:text-blue-600 hover:bg-blue-50 px-5' }} py-3 rounded-full font-bold text-base transition-all duration-300">
                        Competitions
                    </a>
                    @if (auth()->check() && auth()->user()->role === 'peserta')
                        <a href="{{ url('/pesanan') }}" class="{{ request()->is('pesanan') ? 'bg-blue-600 text-white px-6 shadow-lg shadow-blue-500/20' : 'text-slate-600 hover:text-blue-600 hover:bg-blue-50 px-5' }} py-3 rounded-full font-bold text-base transition-all duration-300">
                            Pesanan
                        </a>
                    @endif
                </div>
        ...
        <div class="flex items-center space-x-2 md:space-x-4">
            ...
            @auth
                <div class="flex items-center space-x-2 md:space-x-4">
                    @php
                        // Perbaikan link profil agar Admin tidak terlempar ke halaman penyelenggara
                        $profilUrl = '#';
                        if (auth()->user()->role === 'peserta') {
                            $profilUrl = route('profile.index');
                        } elseif (auth()->user()->role === 'penyelenggara') {
                            $profilUrl = route('penyelenggara.dashboard');
                        } elseif (auth()->user()->role === 'admin') {
                            $profilUrl = route('admin.dashboard');
                        }
                    @endphp
                    <a href="{{ $profilUrl }}" class="flex items-center gap-2 px-4 md:px-6 py-2 md:py-3 bg-blue-50 text-blue-700 rounded-full hover:bg-blue-100 transition font-bold text-xs md:text-base whitespace-nowrap">
    ...
    @if (!auth()->check() || auth()->user()->role !== 'admin')
        <div id="mobile-menu-panel" class="hidden lg:hidden bg-white border-t border-slate-100 shadow-xl absolute w-full left-0 top-full">
            <div class="flex flex-col px-6 py-5 space-y-2">
                ...
                <a href="{{ $dashboardUrl }}" class="{{ $isDashboardActive ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:text-blue-600 hover:bg-blue-50' }} px-4 py-3 rounded-xl font-bold text-[15px] transition-colors block">
                    Dashboard
                </a>
                <a href="{{ $competitionsUrl }}" class="{{ $isCompetitionsActive ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:text-blue-600 hover:bg-blue-50' }} px-4 py-3 rounded-xl font-bold text-[15px] transition-colors block">
                    Competitions
                </a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Pastikan elemen ada sebelum menambahkan event listener (mencegah error di halaman admin)
        if (btn && panel) {
