...
<body class="bg-slate-50 min-h-screen relative pb-20">
    ...
    <main x-data="lombaBuilder()"
        ...
        <div class="w-full lg:w-3/5 bg-white rounded-3xl p-8 shadow-sm border border-slate-200">
            <div class="mb-6 border-b border-slate-100 pb-4">
                <h1 class="text-2xl font-extrabold text-slate-900">Kelola Lomba</h1>
                <p class="text-slate-500 text-sm mt-1">Ubah data di bawah ini. Tampilan kartu akan terupdate otomatis.
                </p>
            </div>
            ...
            <form action="{{ route('penyelenggara.update', $lomba->id) }}" method="POST" enctype="multipart/form-data"
                class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1">Upload Poster (Opsional)</label>
                            <input type="file" name="poster" accept="image/*" @change="handlePoster"
                                class="w-full text-sm text-slate-500 border border-slate-200 rounded-xl p-2.5 bg-slate-50 focus:ring-blue-500">
                            <p class="text-[10px] text-slate-400 mt-1">*Abaikan jika tidak ingin mengubah poster</p>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1">Link Google Drive Juknis
                                *</label>
                            <input type="url" name="link_panduan" x-model="gdrive"
                ...
                <div class="space-y-6">
                            ...
                            @foreach ($categories as $value => $label)
                                <label class="relative cursor-pointer h-full">
                                    <input type="radio" name="kategori" value="{{ $value }}"
                                        class="peer sr-only"
                                        {{ old('kategori', $lomba->kategori) == $value ? 'checked' : '' }} required>
                                    <div
                            ...
                            @foreach ($levels as $value => $label)
                                <label class="relative cursor-pointer h-full">
                                    <input type="radio" name="tingkat_sekolah" value="{{ $value }}"
                                        class="peer sr-only"
                                        {{ old('tingkat_sekolah', $lomba->tingkat_sekolah) == $value ? 'checked' : '' }}
                                        required>
                                    <div
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select x-model="tingkat" disabled
                                class="w-full text-sm text-slate-500 border border-slate-200 rounded-xl p-2.5 bg-slate-100 cursor-not-allowed opacity-70">
                            ...
                            <input type="hidden" name="tingkat_lomba" :value="tingkat">
                            ...
                            <p class="text-[10px] text-red-500 mt-1.5 font-bold flex items-center gap-1">
                                ...
                                Tingkat wilayah lomba tidak dapat diubah setelah dibuat.
                            </p>
                        </div>
                        <div>
        ...
        <div class="w-full lg:w-2/5 lg:sticky lg:top-32">
            ...
            @if ($lomba->status === 'draf')
                @php
                    $tingkatLower = strtolower($lomba->tingkat_lomba);
                    $hargaPublikasi = 0;
                    if (in_array($tingkatLower, ['kota', 'umum'])) {
                        $hargaPublikasi = 25000;
                    }
                    if ($tingkatLower == 'provinsi') {
                        $hargaPublikasi = 50000;
                    }
                    if ($tingkatLower == 'nasional') {
                        $hargaPublikasi = 100000;
                    }
                    if ($tingkatLower == 'internasional') {
                        $hargaPublikasi = 250000;
                    }
                @endphp

                <div
                    class="mt-8 bg-white rounded-[24px] border border-blue-200 shadow-xl overflow-hidden relative w-full max-w-sm mx-auto">
                    <div class="bg-blue-600 text-white text-center py-3">
                        <h4 class="text-sm font-bold uppercase tracking-widest">Selesaikan Pembayaran</h4>
                    </div>
                    <div class="p-6 text-center">
                        <p class="text-sm text-slate-500 mb-2">Tagihan Publikasi Tingkat {{ ucfirst($tingkatLower) }}
                        </p>
                        <p class="text-2xl font-black text-slate-900 mb-4">Rp
                            {{ number_format($hargaPublikasi, 0, ',', '.') }}</p>

                        <div class="border-2 border-dashed border-slate-200 p-2 w-32 h-32 mx-auto mb-5 rounded-xl">
                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=PembayaranWinly&color=0f172a"
                        ...
                        <form action="{{ route('penyelenggara.confirmPayment', $lomba->id) }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="w-full bg-green-500 hover:bg-green-600 text-white text-sm font-bold py-3 rounded-full shadow-lg shadow-green-200 transition-all">
    ...
    @php
        // 1. Olah Data Bidang
        $bidangsData = $lomba->fields
            ->map(function ($field) {
                return [
                    'id' => $field->id,
                    'nama' => $field->nama_bidang,
                    'harga' => $field->harga,
                    'wa' => $field->link_wa,
                ];
            })
        ...
        // 2. Olah Data Fasilitas Lomba (Benefits) Anti-Badai
        $rawBenefits = $lomba->benefits;

        if (is_array($rawBenefits)) {
            $benefitsData = $rawBenefits;
        } else {
            $benefitsData = json_decode($rawBenefits, true);
        }
        ...
        $tingkatData = trim(strtolower($lomba->tingkat_lomba));
    @endphp

    <script>
        function lombaBuilder() {
            return {
                judul: @json($lomba->judul_lomba),
                tanggal: @json($lomba->tanggal_pelaksanaan),
                gdrive: @json($lomba->link_panduan ?? ''),
                posterPreview: @json($lomba->poster ? asset('storage/' . $lomba->poster) : null),
                tingkat: @json($tingkatData),
                benefits: @json($benefitsData),
